...
# Builds a Docker service using the Dockerfile located in the current directory.
#
# Usage: docker::build_service
#
docker::build_service() {
    local -r STACK_NAME="${1:?$(missing_param "build_service" "STACK_NAME")}"
    local -r DOCKER_COMPOSE_PATH="${2:?$(missing_param "build_service" "DOCKER_COMPOSE_PATH")}"
    local -r DOCKER_COMPOSE_FILE="${3:?$(missing_param "build_service" "DOCKER_COMPOSE_FILE")}"
    ...
    log info "Building stack $STACK_NAME ..."
    log info "docker-compose \
        -f ${DOCKER_COMPOSE_PATH}/$DOCKER_COMPOSE_FILE \
...
docker::deploy_service() {
    ...
    log info "Deploying ${DOCKER_COMPOSE_PATH}/$DOCKER_COMPOSE_FILE ..."
